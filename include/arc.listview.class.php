...
class ListView
{
    ...
    function MakeHtml($startpage=1, $makepagesize=0, $isremote=0)
    {
        ...
        if ($this->Fields['moresite']) {
            if ($this->TypeLink->TypeInfos['topid']) {
                $top = new TypeLink($this->TypeLink->TypeInfos['topid']);
                $typedir = str_replace($top->TypeInfos['typedir'], $top->TypeInfos['siteurl'], $this->Fields['typedir']);
            } else {
                $typedir = '';
            }
        }
        for($this->PageNo=$startpage; $this->PageNo < $endpage; $this->PageNo++)
        {
            ...
            $makeFile = $this->GetMakeFileRule($this->Fields['id'],'list',$typedir,'',$this->Fields['namerule2']);
            ...
            $makeFileAfter = $this->GetTruePath().$makeFile;

            $makeFileAfter = preg_replace("/\/{1,}/", "/", $makeFileAfter);
            $murl = $this->GetTrueUrl($murl);
            $content = $this->dtp->GetResult();

            $id = $this->TypeLink->TypeInfos['topid'] ? $this->TypeLink->TypeInfos['topid'] : $this->TypeLink->TypeInfos['id'];
            $topTypeLink = new TypeLink($id);
            $param = array(
                'act' => 'arctype',
                'method' => 'make',
                'path' => $murl,
    ...
    function MakePartTemplets()
    {
        ...
        {
            $murl = $this->GetTrueUrl($murl);
            $content = $this->PartView->GetResult();
            $id = $this->TypeLink->TypeInfos['topid'] ? $this->TypeLink->TypeInfos['topid'] : $this->TypeLink->TypeInfos['id'];
            $topTypeLink = new TypeLink($id);
            $param = array(
                'act' => 'arctype',
    ...
    function GetTrueUrl($nurl)
    {
        if($this->Fields['moresite']==1)
        {
            ...
            {
                $nurl = rtrim($this->Fields['siteurl'], '/') . preg_replace("/".str_replace('/', '\/', $this->Fields['sitepath'])."/", '', $nurl, 1);
            }
        }
        return $nurl;
    }
    ...
    function ParseDMFields($PageNo,$ismake=1)
    {
        ...
        foreach($this->dtp->CTags as $tagid=>$ctag)
        {
            ...
            {
                $noshow = true;
                if ($ctag->GetAtt("noshow") == 'topid' && $this->Fields['topid'] == 0) {
                    $noshow = false;
                }
                if ($noshow) {
    ...
    function GetPageListST($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        $tnamerule = $this->GetMakeFileRule($this->Fields['id'],"list",$typedir,$this->Fields['defaultname'],$this->Fields['namerule2']);
        $tnamerule = $cfg_basehost . $tnamerule;

        //获得上一页和主页的链接
        if($this->PageNo != 1)
        {
            $tmp = rtrim(dirname($tnamerule), '/');
            if ($tmp == "\\") {
                $tmp = '';
            }
            if (2 == $this->PageNo) {
                $prepage.="<li><a href='" . $tmp . "/'>上一页</a></li>\r\n";
            } else {
                $prepage.="<li><a href='".str_replace("{page}",$prepagenum,$tnamerule)."'>上一页</a></li>\r\n";
            }
            $indexpage="<li><a href='" . $tmp . "/'>首页</a></li>\r\n";
        }
        else
        {
            $indexpage="<li>首页</li>\r\n";
        ...
        {
            if (1 != $mjj) {
                $optioinurl = str_replace("{page}",$mjj,$tnamerule);
            } else {
                $optioinurl = rtrim(dirname($tnamerule), '/') . '/';
            }
            if($mjj==$this->PageNo)
            {
                $optionlist .= "<option value='". $optioinurl ."' selected>$mjj</option>\r\n";
            }
            else
            {
                $optionlist .= "<option value='". $optioinurl ."'>$mjj</option>\r\n";
            }
        }
