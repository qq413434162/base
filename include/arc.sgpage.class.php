...
class sgpage
{
    ...
    function __construct($aid)
    {
        ...
        if (isset($this->Fields['topid']) && $this->Fields['topid']) {
            $this->Fields['type'] = new TypeLink($this->Fields['topid']);
            $this->Fields['type'] = $this->Fields['type']->TypeInfos;
        }
...
//        $filename = $GLOBALS['cfg_basedir'].$GLOBALS['cfg_cmspath'].'/'.$this->Fields['filename'];
        helper('site');
        global $dsql;
        $filename = '/'.ltrim($this->Fields['filename'], '/');
        $filename = preg_replace("/\/{1,}/", '/', $filename);
        $rs = MakeSgPage($filename, $this->dtp->GetResult(), $this->Fields['topid']);
        $fail = array();
        if ($rs) {
            ...
            if ($fail) {
                $sql = "SELECT `title`, `aid` FROM #@__sgpage WHERE `aid` IN (" . implode(',', array_keys($fail)) . ")";
                $dsql->SetQuery($sql);
                $dsql->Execute('al');
                $fail = array();
                while($row = $dsql->GetArray("al")) {
                    $fail[$row['aid']] = $row['title'];
                }
            }
        }
        if ($fail) {
            ShowMsg("部分生成失败" . implode(',', $fail) . "！");
            return;
        }
