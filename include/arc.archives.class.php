...
class Archives
{
    ...
    function __construct($aid)
    {
        ...
        else
        {
            ...
            $this->TopidTypeLink = new TypeLink($this->TypeLink->TypeInfos['topid']);
            ...
            $this->Fields['topid_typename'] = $this->TopidTypeLink->TypeInfos['typename'];
            ...
            foreach($this->SplitFields as $k=>$v)
            {
                ...
                if($pos>0)
                {
                    ...
                    if($st==""||$st=="副标题"||$st=="分页标题")
                    {
                        ...
                        $tmp = explode('#e#', $this->SplitFields[$k]);
                        if (isset($tmp[1])) {
                            $this->SplitFields[$k] = $tmp[1];
                        }
                        continue;
                    }
                    else
    ...
    function MakeHtml($isremote=0)
    {
        ...
        else
        {
            for($i=1;$i<=$this->TotalPage;$i++)
            {
                ...
                if($i>1)
                {
                    $currentname = $fileFirst."_".$i.".".$this->ShortName;
                    $TRUEfilename = $this->GetTruePath().$currentname;
                }
                else
                {
                    $TRUEfilename = $this->GetTruePath().$filename;
                }
                $currentname = $this->GetTrueArctypeUrl($currentname);
                $this->ParseDMFields($i,1);
                $html = $this->dtp->GetResult();

                $topTypeLink = new TypeLink($this->TypeLink->TypeInfos['topid']);
                $param = array(
                    'act' => 'archives',
                    'method' => 'make',
                    'path' => $currentname,
                    'host' => $topTypeLink->TypeInfos['siteurl'],
                    'content' => $html,
                    'api_key' => $topTypeLink->TypeInfos['siteurl'],
                    'api_secret' => $topTypeLink->TypeInfos['apikey'],
                );
                helper('site');
                $rs = call($param, 'post');
    ...
    function GetTrueArctypeUrl($nurl)
    {
        if($this->TypeLink->TypeInfos['moresite']==1)
        {
            if($this->TypeLink->TypeInfos['sitepath']!='')
            {
                $nurl = preg_replace("/".str_replace('/', '\/', $this->TypeLink->TypeInfos['sitepath'])."/", '', $nurl, 1);
            }
        }
        return $nurl;
    }
    ...
    {
        global $cfg_basehost;
        $base = dirname($this->Fields['arcurl']) . '/';
        if($totalPage==1)
        {
            return "";
        }
        $PageList = "<li>共".$totalPage."页: </li>";
        $nPage = $nowPage-1;
        $lPage = $nowPage+1;
        if($nowPage==1)
        {
        ...
        else
        {
            if($nPage==1)
            {
                $PageList.="<li><a href='".$base.$this->NameFirst.".".$this->ShortName."'>上一页</a></li>";
            }
            else
            {
                $PageList.="<li><a href='".$base.$this->NameFirst."_".$nPage.".".$this->ShortName."'>上一页</a></li>";
            }
        }
        for($i=1;$i<=$totalPage;$i++)
        {
            if($i==1)
            {
                if($nowPage!=1)
                {
                    $PageList.="<li><a href='".$base.$this->NameFirst.".".$this->ShortName."'>1</a></li>";
                }
                else
                {
                    $PageList.="<li class=\"thisclass\">1</li>";
                }
            }
            else
            {
                ...
                if($nowPage!=$i)
                {
                    $PageList.="<li><a href='".$base.$this->NameFirst."_".$i.".".$this->ShortName."'>".$n."</a></li>";
                }
                else
                {
                    $PageList.="<li class=\"thisclass\">{$n}</li>";
                }
            }
        }
        if($lPage <= $totalPage)
        {
            $PageList.="<li><a href='".$base.$this->NameFirst."_".$lPage.".".$this->ShortName."'>下一页</a></li>";
        }
        else
        {
            $PageList.= "<li>下一页</li>";
    ...
    //获得静态页面所有分页列表
    // add By Baob 20120709
    function GetPagebreakAll($totalPage,$nowPage,$aid)
    {
        //echo "总页数:".$totalPage."<br>";//总页数
        //echo "第几页:".$nowPage."<br>";//第几页
        //echo "文章id:".$aid."<br>";//文章id

        if($totalPage==1)
        {
            return "";
        }
        $showpagenum = 9;//每页显示多少条记录，必须是奇数
        $subpagenum = ceil($totalPage/$showpagenum);
        $curpagenum = ceil($nowPage/$showpagenum);//当前第几页

        $PageList = "<li><a>共".$totalPage."页: </a></li>";

        if($nowPage!=1){
            $PageList.="<li><a href='".$this->NameFirst.".".$this->ShortName."' target='_self'>首页</a></li>";
        }
        ...
        else
        {
            if($nPage==1)
            {
                $PageList.="<li><a href='".$this->NameFirst.".".$this->ShortName."' target='_self'>上一页</a></li>";
            }
            else
            {
                $PageList.="<li><a href='".$this->NameFirst."_".$nPage.".".$this->ShortName."' target='_self'>上一页</a></li>";
            }
        }
        ...
        if($totalPage <=  $showpagenum){
            for($i=1;$i<=$totalPage;$i++){
                if($i==1)
                {
                    if($nowPage!=1)
                    {
                        $PageList.="<li><a href='".$this->NameFirst.".".$this->ShortName."' target='_self'>1</a></li>";
                    }
                    else
                    {
                        $PageList.="<li class=\"thisclass\" target='_self'><a>1</a></li>";
                    }
                }
                else
                {
                    ...
                    if($nowPage!=$i)
                    {
                        $PageList.="<li><a href='".$this->NameFirst."_".$i.".".$this->ShortName."' target='_self'>".$n."</a></li>";
                    }
                    else
                    {
                        $PageList.="<li class=\"thisclass\"><a target='_self'>{$n}</a></li>";
                    }
                }
            }
        }else{
            if($nowPage <= ($showpagenum+1)/2+1){
                for($i=1;$i<=$totalPage;$i++)
                {
                    if ($i <= $showpagenum) {
                        if($i==1)
                        {
                            ...
                            {
                                $PageList.="<li class=\"thisclass\"><a target='_self' target='_self'>1</a></li>";
                            }
                        }
                        else
                        {
                        ...
                        $PageList.="<li style=\"display:none;\"><a href='".$this->NameFirst."_".$i.".".$this->ShortName."' target='_self'>{$i}</a></li>";
                    }
                }
            }
            if($nowPage > ($showpagenum+1)/2+1) //&& $nowPage<=(($subpagenum-1)*$showpagenum)+1)
            {
                $jishu = ($showpagenum - 1) / 2;
                $startnum = $nowPage - $jishu;
                $endnum = $nowPage + $jishu;
                for($i=1;$i<=$totalPage;$i++)
                {
        ...
        if($lPage <= $totalPage)
        {
            $PageList.="<li><a href='".$this->NameFirst."_".$lPage.".".$this->ShortName."' target='_self'>下一页</a></li>";
        }
        else
        {
            $PageList.= "<li><a target='_self' href=\"" . rtrim($this->TypeLink->GetOneTypeUrl($this->TypeLink->TypeInfos), '/') . "/\">下一页</a></li>";
        }

        if($nowPage<$totalPage){
            $PageList.="<li><a href='".$this->NameFirst."_".$totalPage.".".$this->ShortName."' target='_self'>末页</a></li>";
        }
        return $PageList;
    }
    ...
    {
        if($this->TotalPage==1 || count($this->SplitTitles)==0)
        {
        ...
        } else if($styleName=='pagetitle') { //add by Baob date:20120619 Cause:添加分页标题
            $revalue = $this->Fields['shorttitle'];
            if (isset($this->SplitTitles[$pageNo-1]) && $this->SplitTitles[$pageNo-1]) {
                $revalue = $this->SplitTitles[$pageNo-1];
            }
            $revalue = str_replace('#', '', $revalue);
        } else if($styleName=='line') { //add by Baob date:20120829 Cause:添加分页标题列表
            if (!isset($this->SplitTitles[1])) return;
            $revalue = array();
            $i = 2;
            if ($pageNo != 1) {
                $revalue[] = "<li><a href='view.php?aid=$aid'>" . $this->Fields['title'] . "</a></li> \r\n";
            } else {
                $revalue[] = "<li>" . $this->Fields['title'] . "</li> \r\n";
            }
            foreach($this->SplitTitles as $k=>$v)
            {
                if($pageNo==$i)
                ...
                {
                    $revalue[] = "<li><a href='view.php?aid=$aid&pageno=$i'>$v</a></li> \r\n";
                }
                $i++;
            }
            $revalue = implode('', $revalue);
        } else
        {
    ...
    function GetPageTitlesST($styleName, $pageNo)
    {
        if($this->TotalPage==1 || count($this->SplitTitles)==0)
        {
            if($styleName=='pagetitle') {
                return $this->Fields['shorttitle'];
            } else {
                return $this->Fields['title'];
            }
        }
        ...
        if($styleName=='link')
        {
            ...
            $revalue = str_replace(array('#', '#e#'), '', $revalue);
        } else if($styleName=='pagetitleheader') { //add by Baob date:20120619 Cause:添加分页标题
            ...
            $i = 2;
            if ($pageNo != 1) {
                $revalue[] = "<li><a href='".$this->NameFirst.".".$this->ShortName."'>" . $this->Fields['title'] . "</a></li> \r\n";
            } else {
                $revalue[] = "<li>" . $this->Fields['title'] . "</li> \r\n";
            }
            foreach($this->SplitTitles as $k=>$v)
            {
                ...
                {
                    $revalue[] = "<li><a href='".$this->NameFirst."_".$i.".".$this->ShortName."'>$v</a></li> \r\n";
                }
                $i++;
            }
            $revalue = implode('', $revalue);
