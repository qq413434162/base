        ...
        $innertext='', $arcid=0, $idlist='', $channelid=0, $limit='', $att='', $order='desc', $subday=0, $noflag='',$tagid='', $pagesize=0, $isweight='N')
{
    ...
    if($idlist=='')
    {
        ...
        if(!empty($typeid) && !in_array($typeid, array('top', 'reid')))
        {
            //指定了多个栏目时，不再获取子类的id
            ...
            if (isset($refObj->Fields['topid'])) {
                $typeid = $refObj->Fields['topid'];
                if (is_numeric($typeid)) {
                    $arr = $dsql->GetOne("SELECT `id` FROM `#@__arctype` WHERE id='$typeid' ");
                    if($arr['id'])
                    {
                        $orwheres[] = ' arc.typeid IN ('.GetSonIds($arr['id']).')';
                    }
                }
            }
        }
        else if ($typeid == 'reid') {
            if (isset($refObj->TypeLink->TypeInfos['reid'])) {
            ...
            if (isset($refObj->ArcID) && $refObj->ArcID) {
                    $orwheres[] = " arc.id <> '{$refObj->ArcID}'";
            }
        }
