...
if($dopost=='upload')
{
	...
	for($i=1; $i <= $totalform; $i++)
	{
		...
		$altname = str_replace(array('"', "'"), '', $altname);
		...
		if(in_array($fs[count($fs)-1], array('php', 'asp','pl','shtml','jsp','cgi'))) {
			continue;
		}
	 ...
	 if($cfg_remote_site=='Y' && $remoteuploads == 1)
	 {
	 	  ...
	 	  $imgHtml .=  "<img width=\"$imgwidthValue\" height=\"$imgheightValue\" src=\"$imgsrcValue\" alt=\"$altname\"/><br />\r\n";
	 } else {
			if($cfg_multi_site=='N')
			{
				$imgHtml .=  "<img width=\"$imgwidthValue\" height=\"$imgheightValue\" src=\"$imgsrcValue\" alt=\"$altname\"/><br />\r\n";
...
function ImageOK()
{
	...
	inImg  = "<img width=\""+ imgwidth + " height=\""+ imgheight +"\" src=\""+ isrc +"\"";
	inImg += "\" alt=\""+ ialt +"\""+ialign+" />";
...
<table width="100%" border="0">
    ...
    <tr>
    	<td>
		<!-- bof added by Deno: swfupload上传图片 -->
        <fieldset class="dv_swfupload">
            <legend>批量上传</legend>

			<div class="dv_txt4alt">
                <label for="txt4alt" class="label_txt4alt">说明文字：</label>&nbsp;（共<span id="rowsCount">0</span>行）
                &nbsp;&nbsp;&nbsp;说明文字位置：<input type="radio" name="wordPos" id="wordPosMid" checked="checked" value="mid">居中
                &nbsp;&nbsp;<input type="radio" name="wordPos" value="left">居左 
                <input type="checkbox" name="trigAddPager2" id="trigAddPager2" value="yes" checked="checked" /><label for="trigAddPager2">是否添加分页符</label>
                <input type="checkbox" name="water" id="yzz_water" value="yes" /><label for="yzz_water">是否添加水印</label><br />                
                <input type="checkbox" name="dd2" id="dd2" value="yes" />生成缩略图 &nbsp;
						缩略图宽
                        ...
                        &nbsp;<input type="checkbox" value="1" name="auto_gen_pic_set_small_pic" id="auto_gen_pic_set_small_pic" />生成图片集专用缩略图
                        <input type="checkbox" value="1" name="auto_gen_empty_text" id="auto_gen_empty_text" />重复说明文字<br>
                        <span style="color:red;">提示:图片的说明可用"|"分割,"|"前面是图片的"alt"元素内容,后面为图片说明.无分割.则默认两者相同.</span>
        ...
        <!-- eof added by Deno: swfupload上传图片 -->
    	</td>
    </tr>
	<tr>
		<td>
...
window.onload = function() {
	var settings = {
		...
		//upload_url: "/upload8jdj_effort.php?domainset=<?php echo $_GET['domainset'];?>",
        upload_url: ("/plus/redirect.php"),
		post_params: {"PHPSESSID" : "<?php echo $_COOKIE['PHPSESSID']; ?>", "dd2" : document.getElementById('dd2').checked, "w2" : document.getElementById('w2').value, "h2" : document.getElementById('h2').value, "domainset" : "<?php echo $_GET['domainset'];?>"},
