...
if($dopost=='upload')
{
      ...
      $remotedir = preg_replace('/[^\/]*\.(jpg|gif|bmp|png)/', '', $remotefile);
       ...
       VALUES ('小图{$dblitfile}','$imgsrcValue','1','$imgwidthValue','$imgheightValue','0','{$imgsize}','{$nowtme}','".$cuserLogin->getUserID()."');
		...
		else
		{
			...
			$imgwidthValue = $sizes[0];
			$imgheightValue = $sizes[1];
			ImageResize($fullfilename,$imgwidthValue,$imgheightValue);
	...
	$imgsrcValue = $bfilename;
	 if($cfg_remote_site=='Y' && $remoteuploads == 1)
	 {
	 	  ...
	 	  $imgHtml .=  "<img width=\"$bimgwidthValue\" height=\"$bimgheightValue\" alt=\"$altname\" src=\"$imgsrcValue\"/>\r\n";
	 } else {
			if($cfg_multi_site=='N')
			{
				// $imgHtml .=  "<img src=\"$imgsrcValue\" width=\"$imgwidthValue\" border=\"0\" height=\"$imgheightValue\" alt=\"$altname\" style=\"cursor:pointer\" onclick=\"window.open('$urlValue')\" /><br />\r\n";
...
function ImageOK()
{
	...
	// inImg  = "<img src=\""+ isrc +"\" width=\""+ imgwidth . "\" height=\""+ imgheight +"\" border=\""+ iborder +"\" alt=\""+ ialt +"\""+ialign+" />";
	inImg  = "<img width=\""+ imgwidth + "\" height=\""+ imgheight +"\" alt=\""+ ialt +"\""+ialign+" src=\""+ isrc +"\" />";
