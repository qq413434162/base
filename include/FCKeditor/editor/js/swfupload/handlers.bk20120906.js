...
function queueComplete(numFilesUploaded) {
	...
	for (var i=0, j = fileUploaded8swfUpload.length; i<j; ++i) {
        ...
        html = "<p><img width='" + width + "' height='" + height + "'  "+ (alt? ("alt='"+alt+"' "): '') +"src='"+ (document.getElementById('dd2').checked?(url.replace('.jpg', '_lit.jpg').replace('.jpeg', '_lit.jpeg').replace('.gif', '_lit.gif').replace('.png', '_lit.png')):url) +"'/></p>";
        
		if (true === boolTap && i == 0 && j == 1) {
			html = alt + '#e#' + html + '<p>' + img_title +  '</p>';
            ...
            html = '<p>' + img_title_last + '#p#' + alt + '#e#</p>' + html;
		} else if (true === boolTap && (i > 0) && i == j - 1) {
			txt4alt_split_last = txt4alt[i-1].split('|');
			img_title_last = txt4alt_split_last[1] ? txt4alt_split_last[1] : txt4alt_split_last[0];
            html = '<p>' + img_title_last + '#p#' + alt + '#e#</p>' + html + '<p>' + img_title +  '</p>';
		}
        imgHtml.push(html);
