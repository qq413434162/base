...
function queueComplete(numFilesUploaded) {
	...
	var txt4alt_split, alt, img_title, url, width, hegiht;
	...
	for (var i=0; i<fileUploaded8swfUpload.length; ++i) {
        ...
        html = "<p><img width='" + width + "' height='" + height + "'  "+ (txt4alt[i]? ("alt='"+alt+"' "): '') +"src='"+ (document.getElementById('dd2').checked?(url.replace('.jpg', '_lit.jpg').replace('.jpeg', '_lit.jpeg').replace('.gif', '_lit.gif').replace('.png', '_lit.png')):url) +"' border='0' />"+ (img_title? (boolMid?('<br/>'+img_title):('<p style="text-align:left;">'+txt4alt[i]+'</p>')): '<br/>') +"</p>";
        if (true === boolTap && i > 0) {
            html = '#p#' + alt + '#e#' + html;
        }
        imgHtml.push(html);
