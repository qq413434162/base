...
function queueComplete(numFilesUploaded) {
	...
	for (var i=0; i<fileUploaded8swfUpload.length; ++i) {
		var image_info = fileUploaded8swfUpload[i].split('+');
		fileUploaded8swfUpload[i] = image_info[image_info.length - 1];
        ...
        html = "<p><img "+ (txt4alt[i]? ("alt='"+alt+"' "): '') +"src='"+ (document.getElementById('dd2').checked?(fileUploaded8swfUpload[i].replace('.jpg', '_lit.jpg').replace('.jpeg', '_lit.jpeg').replace('.gif', '_lit.gif').replace('.png', '_lit.png')):fileUploaded8swfUpload[i]) +"' border='0' />"+ (img_title? (boolMid?('<br/>'+img_title):('<p style="text-align:left;">'+txt4alt[i]+'</p>')): '<br/>') +"</p>";
        if (true === boolTap && i > 0) {
            html = '#p#' + alt + '#e#' + html;
        }
        imgHtml.push(html);
