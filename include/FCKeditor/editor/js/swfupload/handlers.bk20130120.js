...
function queueComplete(numFilesUploaded) {
	...
	for (var i=0, j = fileUploaded8swfUpload.length; i<j; ++i) {
        ...
        html = "<img width='" + width + "' height='" + height + "'  "+ (alt? ("alt='"+alt+"' "): '') +"src='"+ (document.getElementById('dd2').checked?(url.replace('.jpg', '_lit.jpg').replace('.jpeg', '_lit.jpeg').replace('.gif', '_lit.gif').replace('.png', '_lit.png')):url) +"'/>";
