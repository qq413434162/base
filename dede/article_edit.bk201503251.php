...
if($dopost!='save')
{
    ...
    helper('site');
    $domainset = GetDomain($arcRow['typeid']);
...
else if($dopost=='save')
{
    ...
    $oldArc = $dsql->getOne($sql);
    if ($oldArc['arcrank'] != $arcrank && $oldArc['arcrank'] <= -1 && $arcrank > -1) {
        $sql = "UPDATE `#@__archives` SET auditdate='" . time() ."' WHERE id='$id' ";
