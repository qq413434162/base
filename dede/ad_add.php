...
if($dopost=="save")
{
    $adnames = explode(',', $adname);
    $adnames = array_unique($adnames);
    if (empty($adnames)) {
        ShowMsg("广告位名称为空！","-1");
        exit();
    }
    ...
    foreach($typeids as $key => $typeid) {
        $adname = isset($adnames[$key]) ? $adnames[$key] : $adname;
    //timeset tagname typeid normbody expbody
    $tagname = "banner{$typeid}-" . $clsid;
    $sql = "SELECT typeid FROM #@__myad WHERE typeid='$typeid' AND tagname LIKE '$tagname'";
    $row = $dsql->GetOne($sql);
    if(is_array($row))
    {
