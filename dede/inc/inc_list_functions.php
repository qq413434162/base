...
// 获取栏目名称
function GetTypename2($tid)
{
    global $dsql;
    if (empty($tid)) return '';
    $typename = '';
    $row = $dsql->GetOne("SELECT typename, reid FROM #@__arctype WHERE id = '{$tid}'");
    $typename = isset($row['typename']) ? $row['typename'] : '';
    if ($row && $row['reid'] > 0) {
        $row1 = $dsql->GetOne("SELECT typename FROM #@__arctype WHERE id = '{$row['reid']}'");

        $typename = (isset($row1['typename']) ? $row1['typename'] : '') . ' > ' . $typename;
    }
    return $typename;
}
