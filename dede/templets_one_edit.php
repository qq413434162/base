...
$dsql->SetQuery($sql);
$dsql->Execute('al');
$topid_select = array('<option value="0" ' . ($row['topid'] == 0 ? 'selected="selected"' : '') . '>默认所有站点</option>');
while($r = $dsql->GetArray("al")) {
    $selected = $row['topid'] == $r['id'] ? 'selected="selected"' : '';
    $topid_select[] = '<option value="' . $r['id'] . '" ' . $selected . '>' . $r['typename'] . '</option>';
}
